...
class Receta:
    ...
    def validar_formulario_recetas (data):
        ...
        if data ['fecha_elaboracion'] == "":
            es_valido = False
            flash("Debes proporcionar la fecha de elaboracion de la receta.", "error_fecha_elaboracion")
        if "menos_treinta" not in data:
            es_valido = False
            flash("Debes informar si la receta se elabora en menos de 30 min.", "error_menos_treinta")
        return es_valido
