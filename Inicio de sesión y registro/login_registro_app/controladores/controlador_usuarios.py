...
def nuevo_usuario():
    ...
    if Usuario.validar_registro(data) == False:
        ...
        return redirect('/dashboard') # Redirecciona a la pagina dashboard
...
def procesa_login():
    ...
    if usuario == None:
        ...
        if not bcrypt.check_password_hash(usuario.password, request.form['password_login']): #Si el usuario si existe se hace el check_password_hash. 
